...
def build_run_summary() -> str:
    ...
    lines = [
        ...
        f"- review_bucket_unique: {int(normalize_series(accounts_master_df, 'review_bucket').nunique()) if not accounts_master_df.empty and 'review_bucket' in accounts_master_df.columns else 0}",
        f"- review_bucket_largest: {normalize_series(accounts_master_df, 'review_bucket').value_counts().index[0] if not accounts_master_df.empty and 'review_bucket' in accounts_master_df.columns and not normalize_series(accounts_master_df, 'review_bucket').empty else ''}",
        "",
        "Needs Manual Review",
